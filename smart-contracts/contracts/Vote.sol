...
contract Vote {
    ...
    function addDelegators(address _delegatorAddress) public onlyOwner {
        require(owner != _delegatorAddress, "Owner of account can't be delegator");
    ...
    function getWakanda(address _wakanda) external view returns (WakandaVoter memory) {
        return wakandaVoter[_wakanda];
    }

    function registration(address _wakanda) external onlyOwner {
        require(msg.sender != _wakanda, "You are not allowed for these action");
        require(!wakandaVoter[_wakanda].registered, "You are already registered");
        require(wknd.balanceOf(msg.sender) > 0, "Sender doesn't have enough balance");
    ...
    function vote(address _wakandaAddress, uint256 _candidateId, uint256 _amountOfVotes) external onlySender(_wakandaAddress) {
        require(_wakandaAddress != owner, "Owner of contract could not vote");
        require(!wakandaVoter[_wakandaAddress].hasVoted, "These address already voted");
        uint256 wakandaTokenBalance = wknd.balanceOf(_wakandaAddress);
        require(_amountOfVotes > 0 && wakandaTokenBalance > 0 && _amountOfVotes <= wakandaTokenBalance, "Wakanda doesn't have enaugh balance to vote!");
        require(candidates.length > 0, "There is no candidate to vote for");

        wakandaVoter[_wakandaAddress].voteFor = _candidateId;

        if (wakandaTokenBalance == _amountOfVotes && !wakandaVoter[_wakandaAddress].isDelegator) {
    ...
    function delegateVote(address _wakandaAddress, address _delegatorAddress) external onlySender(_wakandaAddress) {
        require(_wakandaAddress != owner && _delegatorAddress != owner, "Owner of contract could not delegate");
        require(_wakandaAddress != _delegatorAddress && !wakandaVoter[_wakandaAddress].isDelegator, "Delegation form delegator to delegator is not allowed");
        require(wakandaVoter[_wakandaAddress].delegateTo == address(0) && wakandaVoter[_wakandaAddress].registered && !wakandaVoter[_wakandaAddress].hasVoted, "Wakanda can't delegate vote");

        uint256 balanceOfWakanda = wknd.balanceOf(_wakandaAddress);
        require(balanceOfWakanda == VOTING_TOKEN, "Only one token could be delegated");

        require(_delegatorAddress != address(0) && wakandaVoter[_delegatorAddress].isDelegator, "Delegator address is not valid");
